 ...
 * twitter.ts
 *
 * Posts date memory images to the official @LemonDates X (Twitter) account
...
/**
 * Posts the date memory image + caption to @LemonDates.
 */
export async function postDateTweet(params: {
  ...
  return {
    tweetId,
    tweetUrl: `https://twitter.com/LemonDates/status/${tweetId}`,
  };
}
